...
import com.nearaid.feature.post.resources.Res
import com.nearaid.feature.post.resources.*
...
fun CreateListingScreen(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        NearAidTopBar(
            title = if (type == ListingType.REQUEST) stringResource(Res.string.create_title_request) else stringResource(Res.string.create_title_offer),
            onBack = onBack,
        )
        ...
        Column(
            ...
            // ---- Category chips ----
            SectionLabel(stringResource(Res.string.field_category_label))
            FlowRow(
            ...
            NearAidTextField(
                ...
                label = if (type == ListingType.REQUEST) stringResource(Res.string.field_title_label_request) else stringResource(Res.string.field_title_label_offer),
            ...
            NearAidTextField(
                ...
                label = stringResource(Res.string.field_quantity_label),
                placeholder = stringResource(Res.string.field_quantity_placeholder),
            )

            // ---- REQUEST-only: Urgency chips ----
            if (type == ListingType.REQUEST) {
                SectionLabel(stringResource(Res.string.field_urgency_label))
                FlowRow(
                    horizontalArrangement = Arrangement.spacedBy(8.dp),
                    verticalArrangement = Arrangement.spacedBy(8.dp),
                ) {
            ...
            if (type == ListingType.OFFER) {
                NearAidTextField(
                    ...
                    label = stringResource(Res.string.field_available_until_label),
                    placeholder = stringResource(Res.string.field_available_until_placeholder),
                )
            }

            // ---- Location note ----
            Text(
                text = stringResource(Res.string.create_location_note),
            ...
            // ---- Submit button ----
            NearAidButton(
                text = if (type == ListingType.REQUEST) stringResource(Res.string.create_submit_request) else stringResource(Res.string.create_submit_offer),
